...
function findSubStringOverlappingCount(mainString, subString) {
  if (typeof mainString !== 'string' || typeof subString !== 'string') {
    throw new Error('Both arguments must be strings.');
  }
  ...
  for (let i = 0; i <= mainLength - subLength; i++) {
    ...
    if (j === subLength) {
      count++;
      // REMOVED i--; to prevent infinite loop
    }
  }
  return count;
}
// Example usage:
console.log(findSubStringOverlappingCount("aaaaa", "aa")); // Output: 4
//---------------------------------------------------------------------
